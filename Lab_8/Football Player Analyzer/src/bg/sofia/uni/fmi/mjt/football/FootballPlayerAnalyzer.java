...
public class FootballPlayerAnalyzer {
    ...
    public Player getHighestPaidPlayerByNationality(String nationality) {
        if (nationality == null || nationality.isBlank()) {
            throw new IllegalArgumentException("getHighestPaidPlayerByNationality: arg is null");
        }

        return players.stream().filter((player -> player.nationality().equals(nationality)))
                .max(Comparator.comparingLong(Player::wageEuro))
                .orElseThrow(() -> new NoSuchElementException("There is not player with nationality " + nationality));
    }
    ...
    public Map<Position, Set<Player>> groupByPosition() {
        return players.stream().reduce(new HashMap<>(), (acc, player) -> {
            for (Position position : player.positions()) {
                acc.putIfAbsent(position, new HashSet<>());
                acc.get(position).add(player);
            }
    ...
    public Optional<Player> getTopProspectPlayerForPositionInBudget(Position position, long budget) {
        if (position == null || budget < 0) {
            throw new IllegalArgumentException("getTopProspectPlayerForPositionInBudget: wrong values");
        }
    ...
    public Set<Player> getSimilarPlayers(Player player) {
        if (player == null) {
            throw new IllegalArgumentException("getSimilarPlayers: arg is null");
        }

        return players.stream().filter(player1 -> player.preferredFoot() == player1.preferredFoot()
                        && Math.abs(player.overallRating() - player1.overallRating()) <= MAX_DIFFERENCE
                        && !player.positions().stream().filter(player1.positions()::contains).toList().isEmpty())
    ...
    public Set<Player> getPlayersByFullNameKeyword(String keyword) {
        if (keyword == null) {
            throw new IllegalArgumentException("getPlayersByFullNameKeyword: arg is null");
        }
